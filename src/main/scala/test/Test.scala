...
package object Program {
  ...
  type ProgramDependencies =
    Accounts with AccountRates with Properties with Date with Incomes
  ////////////////////////////////////
      ...
      for {
        ...
        date match {
          case _: Month | _: Week | _: Day =>
            src.growAccounts.accrueRent.receiveIncomes.payRents.nextPeriod
          case y: Year =>
            (0 until 12).foldLeft(src)((src_, _) => src_.toMonthlyCadence.runSim).toYearlyCadence
        }
      }

    override def solved(src: dataset[ProgramDependencies]): Boolean =
      (src.underlyingAccounts.exists) &&
        src.underlyingAccounts.value
          .map(_.balance)
          .sum > limit
...
object runner {
  ...
  val incomemeta = Seq(
    ...
    TaxableIncome(
      ...
      500,
      1,
      dates(Week(LocalDate.now().plusMonths(6)), LocalDate.now().plusYears(2))
    ),
    ...
    TaxableIncome(
      ...
      1500,
      1,
      dates(Month(LocalDate.now().plusYears(1)), LocalDate.now().plusYears(10))
    )
  )
  val incomes = Incomes(incomemeta)
  val dat: dataset[ProgramDependencies] = data[ProgramDependencies]()
    .asInstanceOf[dataset[ProgramDependencies]]
