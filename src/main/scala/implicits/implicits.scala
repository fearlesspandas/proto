...
object implicits {
  ...
  class CalcGeneric[A <: dataset[_] with InitialType[_,_]](a: dataset[A])(implicit tagA:ClassTag[A],prov:provider[A]){ // f is not being resolved to anything but the identity function
  ...
  implicit class Fetcher[A <: dataset[_]](a: dataset[A])(implicit tagA:ClassTag[A],prov:provider[A]){
    def fetch[B,U >: A <: dataset[U] with InitialType[B,U]](implicit tagu: ClassTag[U]): dataset[U] with InitialType[B,U] = {
      ...
      instanceu.applyFromData(prov.getOther[U,B]).asInstanceOf[U]
    }
