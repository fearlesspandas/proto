...
////    def set[B <:model[_,B]  with InitialType[initType,B]](implicit tag:ClassTag[B]):dataset[A] => dataset[B] = (d:dataset[A]) => build[B].reset(f(d))
////  }
//  implicit class ToResetter2[initType,A<:dataset[_]](f:dataset[A] => initType){
//    def set[B <:model[_,B  with InitialType[initType,B]]](implicit tag:ClassTag[B]):dataset[A] => dataset[B] = (d:dataset[A]) => build[B].reset2(f(d))
//  }
//
//  implicit class ToResetter3[initType<:dataset[_],A<:dataset[_]](f:dataset[A] => initType){
//    def set[B <:model[_,B  with InitialType[_>:initType,B]]](implicit tag:ClassTag[B]):dataset[A] => dataset[B] = (d:dataset[A]) => build[B].reset2(f(d))
//  }
