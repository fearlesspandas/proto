...
class TMap[dependencies<:dataset[_],mapper<:model[dependencies,_>:dataset[dependencies]<:dataset[_]]](dep:dataset[dependencies], f:mapper) extends dataset[dependencies with TMap[dependencies,mapper]] {
  override val context: contexttype = dep.context
  val dependency = dep
  override def withContext(ctx: contexttype): dataset[dependencies  with TMap[dependencies,mapper]] = new TMap(dep,f) {
