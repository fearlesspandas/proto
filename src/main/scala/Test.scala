...
object Test {
   ...
   */
  val testing = 1d
  class vartest extends rsim[Double,vartest,vartest](
    ((src:dataset[vartest]) => src.fetch[Double,vartest] + testing).set[vartest]
  )(0d)
  class X extends rsim[Double,X,X](X_func)(1d)
  val Y_f = (src:dataset[Y with A]) => {
    ...
    //println("Y state:" + src.fetchFromState[Double,Y](15).value)
    //all operations are typechecked to ensure all datasets have correct initial types
    ((x*x) + a)/(x*2)
  }
  val Y_func = Y_f.set[Y]
  class Y extends rsim[Double,Y with A,Y](Y_func)(1d)
  /**
